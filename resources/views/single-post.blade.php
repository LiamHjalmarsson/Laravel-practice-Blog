{{-- Passing a simple strig of text the : is not needed  its only when pass a php vaible --}}
<x-layout :doctitle="$post->title"> 

    <div class="container py-md-5 container--narrow">
        <div class="d-flex justify-content-between">
            <h2> {{ $post->title }}</h2>
            {{-- If current user can update this ppost --}}
            @can('update', $post)
            <span class="pt-2">
        ...
        <div class="body-content">
            {{ $post->body }}
            {!! $post->body !!} 
            {{-- Telling blade that it dosent have to protect us dont use this every where.. Render it as html CAN BE DANGERURES--}}
        </div>
    </div>
